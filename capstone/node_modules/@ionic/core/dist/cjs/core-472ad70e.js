...
const config = require('./config-779e9925.js');
...
const patchBrowser = async () => {
    // @ts-ignore
    const importMeta = (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('core-472ad70e.js', document.baseURI).href));
...
const global0 = () => {
    ...
    // Setup platforms
    config.setupPlatforms(win);
    // create the Ionic.config from raw config object (if it exists)
    // and convert Ionic.config into a ConfigApi that has a get() fn
    const configObj = Object.assign({}, config.configFromSession(win), { persistConfig: false }, Ionic.config, config.configFromURL(win));
    ...
    Ionic.mode = mode = config.config.get('mode', (doc.documentElement.getAttribute('mode')) || (config.isPlatform(win, 'ios') ? 'ios' : 'md'));
    config.config.set('mode', mode);
